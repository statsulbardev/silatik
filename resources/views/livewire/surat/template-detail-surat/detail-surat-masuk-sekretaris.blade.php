                    ...
                    {{-- @if ($surat->relasiDisposisi->unit_fungsi_teknis)
                        ...
                        @if (count($teknis) > 1)
                            @foreach ($teknis as $index => $item)
                                <tr>
                                    <td>
                                        {{ \App\Models\UnitFungsi::find((int) $item['unit_koordinator'])->nama }}
                                    </td>
                                    <td>
                                        @if (count($teknis->pluck('unit_penerima')[$index]) > 1)
                                            <ul class="pl-3">
                                                @foreach ($teknis->pluck('unit_penerima')[$index] as $item)
